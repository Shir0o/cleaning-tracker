...
void main() {
  ...
  testWidgets(
    'AddTaskPage shows custom interval fields when CUSTOM is selected',
    (WidgetTester tester) async {
      ...
      expect(
        ...
        findsNWidgets(3),
      ); // Name, category, and custom interval field
      expect(find.text('14'), findsOneWidget); // Default value
      expect(find.text('DAYS'), findsOneWidget); // Default unit
    },
  );

  testWidgets('AddTaskPage allows changing custom interval unit', (
    WidgetTester tester,
  ) async {
    tester.view.physicalSize = const Size(800, 1200);
    tester.view.devicePixelRatio = 1.0;
